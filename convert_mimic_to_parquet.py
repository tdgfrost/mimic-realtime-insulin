...
parser.add_argument('--path', type=str, help="Path to `mimic` data directory")


def convert_mimic_to_parquet(path: str):
    # Check if all target files exist and raise an error if not
    filenames = ['hosp/admissions', 'hosp/patients', 'hosp/emar', 'hosp/emar_detail', 'hosp/prescriptions',
                 'hosp/diagnoses_icd', 'icu/chartevents', 'icu/d_items', 'icu/inputevents']
    check_file_exists(path, *filenames)

    # Define the target directory
    target_path = './data/mimic'
...
def convert_file(path, target_path, filename, d_types):
    ...
    (
        ...
        .drop('rawdata', strict=False)  # <- special exception for data_float_h in sicdb
